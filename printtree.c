...
static void PT_stm(FILE *outFile, TR_stm statement, int de)
{
  ...
  case TR_SEQ:
    ...
    PT_stm(outFile, statement->u.SEQ.right,de+1); fprintf(outFile, ")");
    break;
  case TR_LABEL:
  ...
  case TR_CJUMP:
    ...
    fprintf(outFile, "%s", SB_name(statement->u.CJUMP.false)); fprintf(outFile, ")");
    break;
  case TR_MOVE:
    indent(outFile,de); fprintf(outFile, "MOVE(\n"); PT_tree_exp(outFile, statement->u.MOVE.dst,de+1); 
    fprintf(outFile, ",\n");
    PT_tree_exp(outFile, statement->u.MOVE.src,de+1); fprintf(outFile, ")");
    break;
  case TR_EXP:
...
static void PT_tree_exp(FILE *outFile, TR_exp expression, int de)
{
  ...
  case TR_BINOP:
    indent(outFile,de); fprintf(outFile, "BINOP(%s,\n", bin_opera[expression->u.BINOP.op]); 
    PT_tree_exp(outFile, expression->u.BINOP.left,de+1); fprintf(outFile, ",\n"); 
    PT_tree_exp(outFile, expression->u.BINOP.right,de+1); fprintf(outFile, ")");
    break;
  case TR_MEM:
    indent(outFile,de); fprintf(outFile, "MEM");
    fprintf(outFile, "(\n"); PT_tree_exp(outFile, expression->u.MEM,de+1); fprintf(outFile, ")");
    break;
  case TR_TEMP:
  ...
  case TR_ESEQ:
    ...
    fprintf(outFile, ",\n");
    PT_tree_exp(outFile, expression->u.ESEQ.exp,de+1); fprintf(outFile, ")");
    break;
  case TR_NAME:
