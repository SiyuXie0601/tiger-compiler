...
static char str_operator[][4] = {
  "+", "-", "*", "/", 
  "=", "<>", "<", "<=", ">", ">="};
static Ty_ty transTyHeader(AST_ty a);
static Ty_tyList makeFormalTyList(S_table tenv, AST_fieldList params);
...
static void createEscapeList(U_boolList *formal_escs, AST_fieldList fList);
...
F_fragList SEM_transProg(AST_exp exp) {
	init();
...
#define LESS_ARGS 1
#define MORE_ARGS 2
struct expty transExp(Tr_level level, S_table venv, S_table tenv, AST_exp a){
	//For common use	
	int i;
	struct expty expty;
	AST_exp exp;
	...
	Ty_field tyField;
	AST_efieldList eFields;
	AST_efield a_efield;
	int cnt;

	//AST_ifExp, AST_whileExp
	struct expty true_exp;
	struct expty false_exp;
	struct expty test_exp;
	bool if_then_else;

	//AST_forExp, AST_whileExp
	S_symbol var_sym;
...
Ty_tyList tyList;
	...
	/*
	 * AST_varExp, AST_nilExp, AST_intExp, AST_stringExp, AST_callExp,
	 * AST_opExp, AST_recordExp, AST_seqExp, AST_assignExp, AST_ifExp,
	 * AST_whileExp, AST_forExp, AST_breakExp, AST_letExp, AST_arrayExp
	 */
	switch(a->kind) {
	case AST_varExp:
	...
	case AST_callExp://struct {S_symbol func; AST_expList args;} call;
debug("AST_callExp");
...
debug("  function name: %s", S_name(a->u.call.func));
	...
	case AST_opExp://struct {AST_oper oper; AST_exp left; AST_exp right;} op;
debug("AST_opExp");
		...
		oper = a->u.op.oper;
		switch(oper){//AST_plusOp, AST_minusOp, AST_timesOp, AST_divideOp, AST_eqOp, AST_neqOp, AST_ltOp, AST_leOp, AST_gtOp, AST_geOp
		...
		break;
	case AST_recordExp://struct {S_symbol typ; AST_efieldList fields;} record;
debug("AST_recordExp");
		arg_number == EXACT_ARGS;
...
//debug("             type: %s", z_str_ty[expty.ty->kind]);
	...
	case AST_assignExp://{AST_var var; AST_exp exp;} assign;
debug("AST_assignExp");
	...
	case AST_seqExp://AST_expList seq;
debug("AST_seqExp");
...
//debug(" AST_seqExp subexp allocated %d", (int)te_array[i]);
//Tr_printTrExp(te_array[i]);
					i++;
				}
			}
debug(" AST_seqExp subexps prepared (%d in total)", i);
		...
		return expty;
	case AST_ifExp://struct {AST_exp test, then, elsee;} iff
debug("AST_ifExp");
		...
		return expty;
	case AST_whileExp://struct {AST_exp test, body;} whilee
debug("AST_whileExp");
		...
		//useful for determining the legality of AST_breakExp.
		VL_push(NULL);
	...
	case AST_forExp://struct {S_symbol var; AST_exp lo,hi,body; bool escape;} forr
debug("AST_forExp");
		...
		var_exp = transVar(level, venv, tenv, AST_SimpleVar(a->pos, var_sym));
...
debug("AST_letExp");
		...
		te_array[i] = expty.exp;
debug("AST_letExp: weaving all together");
		...
		return expty;
	case AST_arrayExp://{S_symbol typ; AST_exp size, init;} array;
debug("AST_arrayExp");
...
struct expty transVar(Tr_level level, S_table venv, S_table tenv, AST_var v){
	E_enventry entry;
	...
	switch(v->kind) {//AST_simpleVar, AST_fieldVar, AST_subscriptVar
	case AST_simpleVar:
debug("transVar: AST_simpleVar = %s", S_name(v->u.simple));
...
debug("transVar: AST_fieldVar = %s", S_name(field->name));
...
debug("transVar: AST_subscriptVar");
...
Tr_exp transDec(Tr_level level, S_table venv, S_table tenv, AST_dec d){
	Tr_exp tr_exp;//To be returned
	AST_nametyList types;
	AST_namety type;
	AST_fundecList functions;
	AST_fundec head;
	Ty_ty resultTy;
	Ty_tyList formalTys;
	E_enventry entry;
	...
	switch(d->kind) {//AST_varDec, AST_functionDec, AST_typeDec
	case AST_varDec: //struct {S_symbol var; S_symbol typ; AST_exp init; bool escape;} var;
debug("AST_varDec");
		...
		tr_exp = Tr_assignExp(transVar(level, venv, tenv, AST_SimpleVar(d->pos, d->u.var.var)).exp, e.exp);

		break;
	case AST_functionDec: 
debug("AST_functionDec");
		...
		for(functions = d->u.function; functions!=NULL; functions=functions->tail){
			head = functions->head;
			if(head != NULL){
debug("  AST_funDec put header: %s", S_name(head->name));
		...
		for(functions = d->u.function; functions!=NULL; functions=functions->tail){
			head = functions->head;
			if(head != NULL){
debug("  AST_funDec translate body: %s", S_name(head->name));
...
debug("AST_typeDec");
		...
		//Prepare for 3) - see below
		nameTys = checked_malloc(sizeof(AST_ty)*names);
...
/*
static void _createEscapeList(U_boolList *formal_escs, AST_fieldList fList){
	AST_fieldList fl;
	int i = 0;
...
/* 
 * Translate AST_ty to Ty_ty. Only generates an empty object of the corresponding
...
static Ty_ty transTyHeader(AST_ty a){
	S_symbol sym;
	Ty_ty ty;
	switch(a->kind){//AST_nameTy, AST_recordTy, AST_arrayTy
	case AST_nameTy:
 ...
 */
void transTy(S_table tenv, S_symbol sym, AST_ty a){
	AST_fieldList flist;
